...
@foreach ($akta as $k => $v)
<div class="row ml-2 mr-2 pt-2 pb-2 primary-text-color border border-top-0 border-left-0 border-right-0 d-flex align-items-start item-click" id="{{ $v['id'] }}" @if (!isset($mode)) style=" cursor: pointer;" data-url="{{ route('akta.show', ['id' => $v['id']]) }}" data-id="{{ $v['id'] }}" @endif>
